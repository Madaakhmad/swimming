                        ...
                        <div
                            ...
                            <div class="p-8 space-y-4">
                                ...
                                @if ($event['status_event'] == 'berjalan')
                                    ...
                                    @else
                                        <a href="/register"
                                            class="w-full py-5 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl font-black shadow-xl shadow-orange-100 transition transform hover:-translate-y-1 flex items-center justify-center gap-3 text-center">
                                            JADI MEMBER UNTUK DAFTAR <i data-lucide="user-plus" class="w-5 h-5"></i>
                                        </a>
                                    @endif
                                @else
                                    <button disabled
                    ...
                    <div class="p-8 md:p-10 pt-12">
                        @if ($event['biaya_event'] > 0)
                            <div class="text-center mb-8">
                                <h3 class="text-2xl font-black text-slate-900 italic uppercase leading-tight">Satu Langkah
                                    Lagi</h3>
                                <p class="text-xs text-slate-400 mt-2 font-bold uppercase tracking-widest">Selesaikan
                                    Pembayaran Anda</p>
                            </div>

                            {{-- Pembayaran Box (Info Rekening) --}}
                            <div
                                class="bg-ksc-blue rounded-[2rem] p-6 mb-6 text-white relative overflow-hidden shadow-xl shadow-blue-100">
                                <div class="absolute -right-4 -bottom-4 w-20 h-20 bg-white/5 rounded-full blur-2xl"></div>
                                <div class="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
                                    <span class="text-[10px] font-bold text-blue-200 uppercase tracking-widest">Total Harus
                                        Dibayar</span>
                                    <span class="text-xl font-black text-ksc-accent">Rp
                                        {{ number_format($event['biaya_event'], 0, ',', '.') }}</span>
                                </div>
                                <div class="space-y-3">
                                    <div>
                                        <p class="text-[9px] font-bold text-blue-200 uppercase tracking-widest mb-1">Bank
                                            Transfer (BCA)</p>
                                        <div class="flex items-center gap-3">
                                            <p class="text-lg font-mono font-bold">{{ $event['rekening'] }}</p>
                                            <button type="button"
                                                onclick="navigator.clipboard.writeText('{{ $event['rekening'] }}'); alert('Nomor rekening disalin!');"
                                                class="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg transition">
                                                <i data-lucide="copy" class="w-3.5 h-3.5 text-ksc-accent"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <div>
                                        <p class="text-[9px] font-bold text-blue-200 uppercase tracking-widest mb-1">Atas
                                            Nama</p>
                                        <p class="text-sm font-bold italic uppercase">{{ $event['atas_nama'] }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if ($event['photo'] != null)
                                <div
                                    class="relative w-full p-4 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 mb-6 text-center">
                            ...
                            {{-- INPUT METODE & TOTAL BAYAR (TAMBAHAN BARU) --}}
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                <div>
                                    <label
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block italic mb-2">
                                        Metode Pembayaran
                                    </label>
                                ...
                                <div>
                                    <label
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block italic mb-2">
                                        Total yang Ditransfer (IDR)
                                    </label>
                                    <input type="number" name="total_bayar" required
                                        value="{{ $event['biaya_event'] }}" placeholder="Masukkan nominal"
                                        class="bg-white border-2 border-slate-100 text-slate-900 text-xs font-bold rounded-2xl block w-full p-3.5 outline-none focus:border-ksc-blue transition-all">
                                </div>
                            </div>
                            ...
                            <div class="space-y-3">
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block italic">
                                    Unggah Bukti Transfer (Wajib)
                                </label>
                                <div class="relative group">
                                    <input type="file" name="bukti_pembayaran" id="bukti_input" required
                                    ...
                                    <div id="upload-placeholder"
                                        ...
                                        <div
                                            class="w-12 h-12 bg-white shadow-sm rounded-xl flex items-center justify-center text-slate-400 mb-2 group-hover:text-ksc-blue transition-colors">
                                            <i data-lucide="camera" class="w-6 h-6"></i>
                                        </div>
                                        <p id="file-label" class="text-xs font-black text-slate-700 uppercase">Pilih Foto
                                            Bukti</p>
                                    </div>

                                    {{-- Preview Container --}}
                                    <div id="preview-container"
                                        class="hidden relative rounded-[2rem] overflow-hidden border-2 border-ksc-blue shadow-lg">
                                        ...
                                        <div
                                            ...
                                            <p class="text-white text-[10px] font-bold uppercase tracking-widest">Ganti
                                                Foto</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @else
                            ...
                            <div class="text-center py-10">
                                <div
                                    class="w-24 h-24 bg-blue-50 text-ksc-blue rounded-[2.5rem] flex items-center justify-center mx-auto mb-6 shadow-xl shadow-blue-100 animate-bounce">
                                    <i data-lucide="gift" class="w-12 h-12"></i>
                                </div>
                                <h3 class="text-3xl font-black text-slate-900 italic uppercase">Free Access!</h3>
                                <p class="text-sm text-slate-500 mt-2 px-6">Event ini gratis. Klik konfirmasi untuk
                                    mengamankan slot Anda.</p>
                            </div>
                        @endif
